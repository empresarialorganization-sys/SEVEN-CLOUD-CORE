...
const handler=createMcpHandler(async(server)=>{
 ...
 },async({sessionHandle}: any)=>{try{const r=await runBrowser(sessionHandle,{v:1,action:"devStatus"},12000);if("error" in r)return err(r.error);if(r.outcome.status!=="completed")return err("dev_timeout");return text({ok:true,result:r.outcome.result});}catch{return err("dev_status_failed");}});
 ...
 server.registerTool("seven_dev_list",{
  ...
  annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},
 },async({sessionHandle,path,limit}: any)=>{const r=await runBrowser(sessionHandle,{v:1,action:"devList",args:{path,limit}},15000);if("error" in r)return err(r.error);return r.outcome.status==="completed"?text({ok:true,result:r.outcome.result}):err("dev_timeout");});
 ...
 server.registerTool("seven_dev_search",{
  ...
  annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},
 },async({sessionHandle,query,path,maxResults}: any)=>{const r=await runBrowser(sessionHandle,{v:1,action:"devSearch",args:{query,path,maxResults}},18000);if("error" in r)return err(r.error);return r.outcome.status==="completed"?text({ok:true,result:r.outcome.result}):err("dev_timeout");});
 ...
 server.registerTool("seven_dev_read",{
  ...
  annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},
 },async({sessionHandle,path,maxBytes}: any)=>{const r=await runBrowser(sessionHandle,{v:1,action:"devRead",args:{path,maxBytes}},15000);if("error" in r)return err(r.error);return r.outcome.status==="completed"?text({ok:true,result:r.outcome.result}):err("dev_timeout");});
 ...
 },async({sessionHandle,path,text:createText,createBackup}: any)=>{const r=await runBrowser(sessionHandle,{v:1,action:"devWrite",args:{path,text:createText,createBackup}},20000);if("error" in r)return err(r.error);return r.outcome.status==="completed"?text({ok:true,result:r.outcome.result}):err("dev_timeout");});
 ...
 },async({sessionHandle,argv,timeoutMs}: any)=>{const r=await runBrowser(sessionHandle,{v:1,action:"devRun",args:{argv,timeoutMs}},35000);if("error" in r)return err(r.error);return r.outcome.status==="completed"?text({ok:true,result:r.outcome.result}):err("dev_timeout");});
 ...
 server.registerTool("seven_dev_process",{
  ...
  annotations:{readOnlyHint:false,destructiveHint:true,idempotentHint:false,openWorldHint:false},
 },async({sessionHandle,operation,argv,processId,tail}: any)=>{const map={start:"devProcessStart",status:"devProcessStatus",logs:"devProcessLogs",stop:"devProcessStop"} as const;const args=operation==="start"?{argv:argv||[]}:{processId:processId||"",tail};const r=await runBrowser(sessionHandle,{v:1,action:map[operation],args},18000);if("error" in r)return err(r.error);return r.outcome.status==="completed"?text({ok:true,result:r.outcome.result}):err("dev_timeout");});
 ...
 },async({sessionHandle}: any)=>{const r=await runBrowser(sessionHandle,{v:1,action:"devGitStatus"},12000);if("error" in r)return err(r.error);return r.outcome.status==="completed"?text({ok:true,result:r.outcome.result}):err("dev_timeout");});
 ...
 },async({sessionHandle,path,maxBytes}: any)=>{const r=await runBrowser(sessionHandle,{v:1,action:"devGitDiff",args:{path:path||null,maxBytes}},12000);if("error" in r)return err(r.error);return r.outcome.status==="completed"?text({ok:true,result:r.outcome.result}):err("dev_timeout");});
 ...
 server.registerTool("seven_vault_search",{
  ...
  annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},
 },async({sessionHandle,...args}: any)=>{if(!(await handleSession(sessionHandle)))return err("invalid_or_expired_session");try{return text({ok:true,assets:await vaultSearch(args)});}catch{return err("vault_search_failed");}});
 ...
 server.registerTool("seven_vault_fetch",{
  ...
  annotations:{readOnlyHint:true,destructiveHint:false,idempotentHint:true,openWorldHint:false},
 },async({sessionHandle,assetId,mode}: any)=>{if(!(await handleSession(sessionHandle)))return err("invalid_or_expired_session");const a=await vaultAsset(assetId);if(!a)return err("asset_not_found");const meta=publicAssetMeta(a);const image=String(a.mime_type).startsWith("image/");
